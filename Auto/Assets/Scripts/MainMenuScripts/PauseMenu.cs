...
public class PauseMenu : MonoBehaviour
{
    ...
    {
        netWorkedClient.GetComponent<NetworkedClient>().SendMessageToServer(11 + ",");
        netWorkedClient.GetComponent<GameLogic>().LevelLoader.GetComponent<LevelLoader>().LoadMainMenuLevel();
    }
    ...
    {
        if (gameObject.GetComponent<GameLoop>().FastForward)
        {
            gameObject.GetComponent<GameLoop>().FastForward = false;
            gameObject.GetComponent<GameLoop>().gameSpeed = 1.0f;
        }
        else
        {
            gameObject.GetComponent<GameLoop>().FastForward = true;
